...
namespace BibleReading.Common45.Root.Security.Cryptography
{
    public class SymmetricCryptography<T> where T : SymmetricAlgorithm, new()
    {
        ...
        {
            return Transform(input, _provider.CreateEncryptor(key, iv));
        }
        ...
        {
            byte[] output = Transform(_utf8.GetBytes(text), _provider.CreateEncryptor(key, iv));
